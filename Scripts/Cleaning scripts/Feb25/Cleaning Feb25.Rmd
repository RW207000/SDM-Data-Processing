...
doses_per_patient <- select(medication_clean, c("coid_asse_id", "assessment_entry"))

doses_per_patient_2 <- doses_per_patient %>%
  group_by(coid_asse_id) %>%
  summarise("number of doses" = max(assessment_entry))

meds_wide <- left_join(meds_wide, doses_per_patient_2, by = c("coid_asse_id_1" = "coid_asse_id"))
