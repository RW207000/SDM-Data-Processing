...
```{r : reading in data}
participants_all <- read.csv("Data/1 Registry extracts/CaHASE2 Feb 25 Extract/Longitudinal data 24.02.2025.csv",header=T, na.strings="")
participants_all$Date.of.Assessment <- as.Date(participants_all$Date.of.Assessment, format = "%d/%m/%Y")

# add in time frame
participants <- participants_all[participants_all$Date.of.Assessment >= "2022-12-01",]

medication <- read.csv("Data/1 Registry extracts/CaHASE2 Feb 25 Extract/Glucocorticoids Data 24.02.2025.csv", header=T, na.strings= c("", "NA"))
medication <- remove_empty(medication, c("cols"), cutoff = 1)

labs_all <- read.csv("Data/1 Registry extracts/CaHASE2 Feb 25 Extract/Labs data 24.02.2025.csv", header=T, na.strings= c("", "NA"))
...
write.csv(participantsnoduplicates, file = "Data/3 Cleaned data/CaHASE2 Feb 25 extract/participantsnoduplicates.csv", row.names = F, na = "")

# medication_clean
write.csv(medication_clean, file = "Data/3 Cleaned data/CaHASE2 Feb 25 extract/medication_clean.csv", row.names = F, na = "")

# meds_wide
write.csv(meds_wide, file = "Data/3 Cleaned data/CaHASE2 Feb 25 extract/meds_wide.csv", row.names = F, na = "")

# CAH biomarkers
# CAH_biomarkers <- CAH_biomarkers[CAH_biomarkers$Date >= '2022-12-01',]
CAH_biomarkers_export <- apply(CAH_biomarkers,2,as.character)
write.csv(CAH_biomarkers_export, file = "Data/3 Cleaned data/CaHASE2 Feb 25 extract/CAH_biomarkers.csv", row.names = F, na = "")

labs_wide_export <- apply(labs_wide,2,as.character)
write.csv(labs_wide_export, file = "Data/3 Cleaned data/CaHASE2 Feb 25 extract/labs_wide.csv", row.names = F, na = "")
```

```{r : merge dataframes}

#CAH_biomarkers_fixed <- mergeBioMarkerEntries(CAH_biomarkers)
