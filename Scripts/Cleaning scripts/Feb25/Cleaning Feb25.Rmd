...
I need to add in various columns using BSA
```{r}
participantsnoduplicates$`BSA (no height available)` <- NA

a <- (0.8168 - (0.0154 * log10(participantsnoduplicates$Weight..kg. * 1000)))
b <- 4.688 * (participantsnoduplicates$Weight..kg. * 1000)^a
participantsnoduplicates$`BSA (no height available)` <- b / 10000
participantsnoduplicates$`BSA (no height available)` <- round(participantsnoduplicates$`BSA (no height available)`, digits = 2)
```
...
```{r : clean absolute doses and add HCe/BSA}
# clean up absolute values (to exclude IM HC values which will otherwise skew totals)
medication_clean$clean_absolute_dose <- 
  ifelse(medication_clean$Medicine == "IM Hydrocortisone", NA, medication_clean$Dose)
```
...
medperpatient <- medication_clean %>%
  ...
  summarise(total_daily_HCeq_dose = sum(HCeq_dose, na.rm = TRUE),
            total_absolute_daily_dose = sum(clean_absolute_dose, na.rm = TRUE))
...
meds_wide <- meds_wide[-c(14,15,22,23,24,27,28,35,36,37,40,41,46,47,48,49,50)]
```

```{r : add in total absolute daily dose}
meds_wide$'total absolute daily dose' <- NA
meds_wide$`total absolute daily dose` <- rowSums(cbind(meds_wide$clean_absolute_dose_1, meds_wide$clean_absolute_dose_2, meds_wide$clean_absolute_dose_3), na.rm = TRUE)
```

```{r : therapy type - single or combined}
meds_wide$`therapy type` <- NA
...
all_data$`HCe total daily dose per BSA` <- all_data$total_daily_HCeq_dose / all_data$`BSA (no height available)`
```

```{r}
# write csv of all_data
write.csv(all_data, file = "Outputs/Dataframe outputs/CaHASE2/all_data_Nov22_Feb25.csv", row.names = F, na = "")
```
